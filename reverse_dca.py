...
class ReverseDCA:
	def __init__(self, binance_client, telegram_bot, ticker, initial_direction, 
		...
		# Crazy Then before placing orders, sync the time
		time_res = self.binance_client.futures_time()
		self.binance_client.timestamp_offset = time_res['serverTime'] - int(time.time() * 1000)
	...
	# Add this function to sync with Binance server time
	def sync_time(self):
	...
	def check_opened_position(self):
		try:
			...
			if len(open_positions) > 0:
				for position in open_positions:
					if position['positionSide'] == 'LONG' and self.initial_direction.lower() == "buy":
						return position
					elif position['positionSide'] == 'SHORT' and self.initial_direction.lower() == "sell":
						return position
			return -1
	...
	def place_market_order(self, size, direction, mark_price):
		...
		print("🍀🍀🍀 position_direction:", direction)
		print("🍀🍀🍀 position_side:", position_side)
		print("🍀🍀🍀 position_mode:", position_mode)
	    

		try:
			print(f"🚩order: symbol: {self.ticker}, side:{direction}, type:{ORDER_TYPE_MARKET},position_side:{position_side}, quantity:{abs(size)}")
			
			# when charged price is low, Set leverage (example: 5x)
			self.binance_client.futures_change_leverage(symbol=self.ticker, leverage=5)
			print(f"futures_change_leverage: {self.binance_client.futures_change_leverage(symbol=self.ticker, leverage=5)}")
	        
			positions = self.binance_client.futures_position_information(symbol=self.ticker)
			# account = self.binance_client.futures_account()
			balance = self.binance_client.futures_account_balance()
			# print(f"positions: {len(positions)},{balance}")
			...
			try:
				...
				print(f"order binance exception", {e}) 
				return []
			
			time.sleep(1)
			print(f"🚩order: {order}")

			print("ℹNumber of position:", len(positions))

			self.telegram_bot.send_message(f"🎯🎯🎯 Created Order\nsymbol: {order['symbol']}\nside  : {order['side']}\npositionSide : {order['positionSide']}\nstatus : {order['status']}\nprice : {order['avgPrice']}\n")
			# print("order:", order)
			positions = self.binance_client.futures_position_information(symbol=self.ticker)
		...
		while True:
			try:
				...
				if e.code == -4045:
					return
				if e.code == -4006:
					self.telegram_bot.send_message(f"Exception! Stop price less than Zero!")	
					return
			except Exception as e:
				self.telegram_bot.send_message(f"Exception! Placing market order. Retrying... {e}")
	...
	def place_initial_tpsl_orders(self):
		if self.initial_direction.lower() == "buy":
			...
			self.take_profit_price = self.first_entry_price * (1 - self.take_profit_pct)
			print("🍀🍀🍀 Take_profit_price",self.take_profit_price)
			stop_loss_price = self.first_entry_price * (1 + self.stop_loss_pct)
			print('sell!!')
	...
	def run(self):
		...
		open_position = self.check_opened_position()
		print(f"🟩 open_position: {open_position},{self.avg_entry_price}")
	
		if open_position != -1:
			print(f"🚩 {self.ticker} Already opened")
			return
		
		self.telegram_bot.send_message("\n Running Reverse DCA Strategy...")
		print(f"\n Running Reverse DCA Strategy...")
		...
		while True:
			...
			if self.avg_entry_price == 0:    # not in position
				...
				print("*********** Inintial direction ------->", self.initial_direction)
				if self.initial_direction.lower() == "buy":
					if (sma_last_closed_candle > current_sma or current_sma == 0) and \
					   (hma1_last_closed_candle > current_hma1 or current_hma1 == 0) and \
					   (hma2_last_closed_candle > current_hma2 or current_hma2 == 0) and \
					   (hma3_last_closed_candle > current_hma3 or current_hma3 == 0):
						...
						leverage = 20
						order_size = round(self.current_volume * leverage / mark_price, self.quantity_precision)
						print("********************* break *************")
					...
					if (sma_last_closed_candle < current_sma or current_sma == 0) and \
					   ...
					   (hma3_last_closed_candle < current_hma3 or current_hma3 == 0) :
						...
						print("********** quantity_precision ---------->", self.quantity_precision)
						print("********** order_size     -------------->", order_size)
						if order_size >= 0 and order_size <= 1:
							self.telegram_bot.send_message(f"⚠⚠⚠ The amount of {self.ticker} is not enough for trading")
							print(f"⚠ The amount of {self.ticker} is not enough for trading")
							# time.sleep(1)
							break
						self.telegram_bot.send_message(f"🎯 {self.ticker}****************************************************\n First entry."
									 				   f"Opening new position with base volume {order_size}")
						exc_order = self.place_market_order(order_size, self.initial_direction, mark_price)			
						self.first_entry_price = self.filled_price
						# if(len(order))
						if(len(exc_order) == 0):
							print(f"⚠ {self.ticker} order is not filled")
							return
						# Place initial take profit and stop loss orders.
						self.place_initial_tpsl_orders()
						print("good!!!!!!!")
